...
fn key_schedule(key: &[u8], mode: u8) -> Vec<[u32; 6]> {
    ...
    for i in 0..16 {
        ...
        for j in 24..48 {
            schedule[togen][j / 8] |=
                bitnum_intr(d, (key_compression[j] - 27) as usize, 7 - (j % 8));
        }
    }
...
fn initial_permutation(input_data: &[u8]) -> (u32, u32) {
    let s0 = bitnum(input_data, 57, 31)
    ...
    let s1 = bitnum(input_data, 56, 31)
...
fn f(state: u32, key: &[u32; 6]) -> u32 {
    ...
    let state = ((SBOX[0][sbox_bit((lrgstate[0] >> 2).into()) as usize] as u32) << 28)
        | ((SBOX[1][sbox_bit((((lrgstate[0] & 0x03) << 4) | (lrgstate[1] >> 4)).into()) as usize]
            as u32)
            << 24)
        | ((SBOX[2][sbox_bit((((lrgstate[1] & 0x0f) << 2) | (lrgstate[2] >> 6)).into()) as usize]
            as u32)
            << 20)
        | ((SBOX[3][sbox_bit((lrgstate[2] & 0x3f).into()) as usize] as u32) << 16)
        | ((SBOX[4][sbox_bit((lrgstate[3] >> 2).into()) as usize] as u32) << 12)
        | ((SBOX[5][sbox_bit((((lrgstate[3] & 0x03) << 4) | (lrgstate[4] >> 4)).into()) as usize]
            as u32)
            << 8)
        | ((SBOX[6][sbox_bit((((lrgstate[4] & 0x0f) << 2) | (lrgstate[5] >> 6)).into()) as usize]
            as u32)
            << 4)
