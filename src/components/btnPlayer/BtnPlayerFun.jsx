...
export const leftClick = async (musicListImportState, data, setData) => {
  ...
  let song = await invoke('play_music', { filePath: audioMeta.audio_src, duration: audioMeta.total_duration, skipSecs: 0 });
...
export const togglePlayPause = async (data, setData) => {
  ...
  if (!data.isPlaying){
    ...
    if (data.totalDuration-data.barCurrentProgressSec<=1 || data.playState===-1) {
      let song = await invoke('play_music', { filePath: data.audioSrc, duration: data.totalDuration, skipSecs: 0 });
