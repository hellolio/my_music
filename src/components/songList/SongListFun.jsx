...
export const importMusic = async (
  ...
  setAllSongList((prevAllSongList) =>
    prevAllSongList.map((item, index) =>
      index === currentIndex
        ? { ...item, songs: item.songs.concat(songs_new) }
        : item
    )
  );
...
export const handleCheckboxChange = (
  ...
  if (!selectedItems.includes(song)) {
    ...
    setSelectedItems((prevSelected) => prevSelected.filter((i) => i !== song));
  }
};
  ...
  setAllSongList((prevAllSongList) =>
    prevAllSongList.map((item, index) =>
      index === currentIndex
        ? {
            ...item,
            songs: item.songs.filter((song) => !selectedItems.includes(song)),
          }
        : item
    )
  );
