...
function LyricsPlayer(props) {
  ...
  useEffect(() => {
    ...
    const index = data.lyrics.findIndex(
      (e) => e.time > data.barCurrentProgressSec
    );
    const newIndex =
      index === -1 ? data.lyrics.length - 1 : Math.max(0, index - 1);
  ...
  return (
    <div className={`${styles.container} ${styles.lyrics}`}>
      <div ref={containerRef} className={styles.centerLyrics}>
        <button
          className={styles.overLoadLyrics}
          onClick={() => addLyrics(data, setData)}
        >
          重载歌词
        </button>
        <ul className={styles.centerLyricsLine}>
